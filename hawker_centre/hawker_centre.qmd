...
```{r}
nb <- include_self(
  st_knn(st_geometry(hk_cp_lclq), 6))
```
...
```{r}
Carpark_hawker <- hk_cp_lclq %>%
  filter(Name == "Carpark")
A_hk <- hk_cp_lclq$Name

Carpark_hdb <- hdb_cp_lclq %>%
  filter(Name == "Carpark")
A_hdb <- hdb_cp_lclq$Name

Carpark_mall <- mall_cp_lclq %>%
  filter(Name == "Carpark")
A_mall <- mall_cp_lclq$Name

# identical(Carpark_hawker, Carpark_hdb)
...
## Computing LCLQ
```{r}
LCLQ_AB <- local_colocation(A, B, nb, wt, 49)

# LCLQ_AC <- local_colocation(A, C, nb, wt, 49)

# LCLQ_AD <- local_colocation(A, D, nb, wt, 49)
```

## write to rds
```{r}
write_rds(LCLQ_AB, "data/rds/lclq_AB.rds")

#write_rds(LCLQ_AB, "data/rds/lclq_AC.rds")

#write_rds(LCLQ_AB, "data/rds/lclq_AD.rds")
```

## read rds
```{r}
LCLQ_AB <- read_rds("data/rds/lclq_AB.rds")

#LCLQ_AC <- read_rds("data/rds/lclq_AC.rds")
#LCLQ_AD <- read_rds("data/rds/lclq_AD.rds")
```
...
```{r}
LCLQ_hawker <- cbind(interest_lclq, LCLQ_AB)
```
